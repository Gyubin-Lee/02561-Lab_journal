window.onload = function init() {
    ...
    var mode = 0; // 0: init, 1: rotate, 2: pan, 3: dolly
    ...
    function initEventHandlers(canvas) {
            ...
            if (dragging) {
                ...
                switch (mode) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        var pan = scale(Math.max(5, 0.25 * dolly_dist), vec2(subtract(lastPos, currentPos)));
                        pan_offset = add(pan_offset, pan);
                        break;
                }
                lastX = x;
                lastY = y;
        ...
        canvas.onwheel = function (ev) {
            var delta = ev.deltaY;
            dolly_dist = Math.min(30, Math.max(1, dolly_dist + 0.01 * delta));
        };
    }
    ...
    function render() {
        ...
        let forward = qrot.apply(vec3(0, 0, -1));

        let rotateEye = qrot.apply(vec3(0, 0, 4));
        let look_at = add(scale(pan_offset[0], right), scale(pan_offset[1], up));
        rotateEye = add(add(rotateEye, look_at), scale(dolly_dist, forward));
 
        var viewMatrix = lookAt(rotateEye, look_at, up);
